...
import { FileText, Database, ShieldAlert, Sparkles, Check, ArrowLeft, Copy, Download, Layers, ShieldCheck, RefreshCw } from 'lucide-react';

interface DeveloperHubProps {
  onBack: () => void;
}

export default function DeveloperHub({ onBack }: DeveloperHubProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-1 sm:px-6 lg:px-8 py-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pb-4 border-b border-slate-205">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1.5">View, download, and copy production databases, variables, and compliance manuals</p>
          </div>
        </div>
        <button 
          onClick={onBack}
          className="bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs px-5 py-3 rounded-xl transition-all shadow"
        >
          Back to Client Home
        </button>
      </div>
        ...
        <div className="lg:col-span-3 bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xs flex flex-col justify-between">
          ...
          <div className="bg-slate-50 border-b border-slate-150 py-3 px-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handleCopy}
                className="bg-white hover:bg-slate-100 text-slate-700 font-bold text-xs py-2 px-3 rounded-lg border border-slate-200 transition-all flex items-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                ...
                className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs py-2 px-3 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer"
              >
