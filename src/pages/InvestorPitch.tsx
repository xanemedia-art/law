...
const SLIDES = [
  ...
  },
  {
    ...
    accent: 'from-rose-500 to-red-650'
  },
  {
    id: 'solution',
    ...
    points: [
      {
        title: 'On-Demand Metred Consultations',
    ...
    accent: 'from-indigo-500 to-violet-650'
  },
  {
    id: 'tech',
...
export default function InvestorPitch({ theme, onToggleTheme }: { theme: 'light' | 'dark'; onToggleTheme: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col justify-between font-sans selection:bg-amber-500/30 overflow-x-hidden relative">
      ...
      <header className="px-6 py-4 flex items-center justify-between border-b border-slate-900 bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <Link to="/" className="text-slate-400 hover:text-white transition-colors p-2 rounded-lg hover:bg-slate-900">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div className="flex items-center gap-2">
            <div className="bg-gradient-to-br from-amber-500 to-yellow-600 text-white p-2 rounded-xl">
              <Scale className="w-4 h-4" />
            </div>
            <div>
              <span className="font-display font-black text-sm tracking-tight block leading-none">LEGALTALK</span>
              <span className="text-[8px] font-bold tracking-widest text-amber-500 block uppercase font-mono mt-0.5">Investor Deck</span>
            </div>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-1.5 bg-slate-900 border border-slate-800 rounded-full px-3 py-1 text-xs font-semibold text-slate-400">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
            <span>Interactive Deck V1.0</span>
          </div>
          <button
            onClick={() => setIsPlaying(!isPlaying)}
            className="p-2 rounded-full border border-slate-800 bg-slate-900 hover:bg-slate-800 text-slate-350 hover:text-white transition-colors cursor-pointer"
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-8 flex flex-col justify-center relative z-10">
        ...
        {/* SLIDE CARD */}
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 sm:p-10 shadow-2xl relative min-h-[480px] flex flex-col justify-between backdrop-blur-xl">
          
          {/* Progress Indicator */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-slate-850 rounded-t-3xl overflow-hidden">
            <motion.div 
              className="h-full bg-gradient-to-r from-amber-500 via-indigo-500 to-emerald-500"
              initial={{ width: '0%' }}
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {SLIDES[currentSlideIndex].id === 'cover' && (
                  <div className="text-center max-w-3xl mx-auto space-y-6">
                    <motion.div 
                      ...
                      className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 text-amber-500 px-4 py-1.5 rounded-full text-xs font-bold font-mono tracking-wide"
                    >
                      <Sparkles className="w-3.5 h-3.5" />
                      <span>REVOLUTIONIZING INDIAN LEGAL TECH</span>
                    </motion.div>
                    
                    <h1 className="font-display font-black text-4xl sm:text-6xl tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-400 bg-clip-text text-transparent">
                      {SLIDES[currentSlideIndex].title}
                    </h1>
                    
                    <p className="text-lg sm:text-xl font-bold text-amber-455">
                    ...
                    <div className="pt-6 flex justify-center gap-4">
                      <button 
                        onClick={handleNext}
                        className="bg-white hover:bg-slate-100 text-slate-950 font-bold text-xs uppercase tracking-wider px-6 py-3.5 rounded-xl transition-all shadow-lg flex items-center gap-2 cursor-pointer border border-white"
                      >
                ...
                {SLIDES[currentSlideIndex].id === 'problem' && (
                  <div className="space-y-6">
                    <div className="text-center sm:text-left">
                      <span className="text-[10px] font-bold tracking-widest text-rose-500 uppercase font-mono">The Market Opportunity</span>
                    ...
                    <div className="grid md:grid-cols-3 gap-6 pt-4">
                      {SLIDES[currentSlideIndex].points?.map((p, idx) => (
                        <div key={idx} className="bg-slate-955/50 border border-slate-850 p-6 rounded-2xl space-y-3 relative hover:border-rose-500/40 transition-colors">
                          <div className="absolute top-0 right-0 w-12 h-12 bg-rose-500/5 rounded-tr-2xl rounded-bl-3xl flex items-center justify-center font-bold text-rose-500 font-mono text-sm border-l border-b border-slate-850">
                            0{idx + 1}
                          </div>
                          <h3 className="font-bold text-base text-rose-400 pr-6">{p.title}</h3>
                ...
                {SLIDES[currentSlideIndex].id === 'solution' && (
                  <div className="space-y-6">
                    <div className="text-center sm:text-left">
                      <span className="text-[10px] font-bold tracking-widest text-indigo-400 uppercase font-mono">The Disruptive Innovation</span>
                      <h2 className="font-display font-black text-2xl sm:text-4xl text-white mt-1">
                        {SLIDES[currentSlideIndex].subtitle}
                      </h2>
                    </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-6 pt-4">
                      {SLIDES[currentSlideIndex].points?.map((p, idx) => (
                        <div key={idx} className="bg-slate-955/50 border border-slate-850 p-6 rounded-2xl space-y-3 relative hover:border-indigo-500/40 transition-colors">
                          <div className="absolute top-0 right-0 w-12 h-12 bg-indigo-500/5 rounded-tr-2xl rounded-bl-3xl flex items-center justify-center font-bold text-indigo-400 font-mono text-sm border-l border-b border-slate-850">
                            0{idx + 1}
                          </div>
                          <h3 className="font-bold text-base text-indigo-400 pr-6">{p.title}</h3>
                          <p className="text-xs text-slate-400 leading-relaxed font-medium">{p.desc}</p>
                        </div>
                      ))}
                    </div>
                ...
                {SLIDES[currentSlideIndex].id === 'tech' && (
                  <div className="grid md:grid-cols-12 gap-8 items-center">
                    <div className="md:col-span-5 space-y-4">
                      <span className="text-[10px] font-bold tracking-widest text-emerald-400 uppercase font-mono">Built for Scale</span>
                      ...
                      <div className="bg-emerald-950/20 border border-emerald-900/40 p-4 rounded-xl flex items-start gap-3">
                        ...
                        <div>
                          <strong className="block text-xs font-bold text-white">Bar Council Adherent</strong>
                          <span className="block text-[10px] text-slate-450 leading-relaxed">Fully aligned with Indian advocate marketing rules by offering directory index services without active solicitation.</span>
                        </div>
                      </div>
                    </div>

                    <div className="md:col-span-7 bg-slate-955/80 border border-slate-850 rounded-2xl p-6 space-y-4">
                      <h3 className="font-mono text-xs font-bold text-slate-500 uppercase tracking-widest">Technological Modules</h3>
                      <div className="space-y-3">
                        {SLIDES[currentSlideIndex].details?.map((d, idx) => (
                          <div key={idx} className="flex justify-between items-center py-2.5 border-b border-slate-900 last:border-0">
                            <span className="text-xs text-slate-400 font-medium">{d.area}</span>
                ...
                {SLIDES[currentSlideIndex].id === 'business-model' && (
                  <div className="space-y-6">
                    <div className="text-center sm:text-left">
                      <span className="text-[10px] font-bold tracking-widest text-blue-400 uppercase font-mono">Economics of LegalTalk</span>
                      <h2 className="font-display font-black text-2xl sm:text-4xl text-white mt-1">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 pt-4">
                      {SLIDES[currentSlideIndex].models?.map((m, idx) => (
                        <div key={idx} className="bg-slate-955/50 border border-slate-850 p-6 rounded-2xl flex flex-col justify-between hover:border-blue-500/40 transition-colors">
                          <div className="space-y-3">
                            <h3 className="font-bold text-sm text-slate-350 uppercase tracking-wide">{m.title}</h3>
                            <span className="block text-xl font-display font-black text-blue-405">{m.rate}</span>
                            <p className="text-xs text-slate-400 leading-relaxed font-medium">{m.detail}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {SLIDES[currentSlideIndex].id === 'sandbox' && (
                  <div className="space-y-6">
                    <div className="text-center sm:text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div>
                        <span className="text-[10px] font-bold tracking-widest text-amber-500 uppercase font-mono">Live Interactive Demo</span>
                      ...
                      <div className="flex gap-2">
                        <Link to="/login" className="bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all flex items-center gap-2">
                    ...
                    <div className="grid lg:grid-cols-12 gap-6 pt-2">
                      ...
                      <div className="lg:col-span-4 bg-slate-955/50 border border-slate-850 p-6 rounded-2xl space-y-4">
                        ...
                        <div className="space-y-3 text-xs">
                          <div>
                            <div className="flex justify-between mb-1.5">
                              <span className="text-slate-400 font-medium">Daily Consult Minutes</span>
                              <strong className="text-white font-mono">{dailyCalls.toLocaleString()} min</strong>
                            </div>
                            <input 
                              type="range" 
                              min="100" 
                          ...
                          <div>
                            <div className="flex justify-between mb-1.5">
                              <span className="text-slate-400 font-medium">Daily Chats Handled</span>
                              <strong className="text-white font-mono">{dailyChats.toLocaleString()} chats</strong>
                            </div>
                            <input 
                              type="range" 
                              min="100" 
                              ...
                              className="w-full accent-amber-500 cursor-pointer h-1.5 bg-slate-850 rounded-lg"
                            />
                          </div>

                          <div>
                            <div className="flex justify-between mb-1.5">
                              <span className="text-slate-400 font-medium">Active Subscribed Advocates</span>
                              <strong className="text-white font-mono">{activeAdvocates.toLocaleString()} advocates</strong>
                            </div>
                            <input 
                              type="range" 
                              min="50" 
                        ...
                        <div className="border-t border-slate-900 pt-4 space-y-2 text-xs">
                          <div className="flex justify-between items-center text-slate-400">
                            ...
                            <span className="font-bold font-mono text-white">₹{advocateTotalMonthlyEarnings.toLocaleString()}/mo</span>
                          </div>
                          <div className="flex justify-between items-center bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl">
                            <span className="font-bold text-amber-500 text-[10px] uppercase tracking-wider">Net Platform Revenue</span>
                            <span className="font-display font-black text-sm text-amber-450 font-mono">
                      ...
                      <div className="lg:col-span-4 bg-slate-955/50 border border-slate-850 p-6 rounded-2xl space-y-4 flex flex-col justify-between">
                        <div className="space-y-4">
                          ...
                          {mockCallState === 'idle' && (
                            <div className="text-center py-6 space-y-3">
                              ...
                              <button 
                                ...
                                className="bg-indigo-600 hover:bg-indigo-750 text-white font-bold text-xs px-4 py-2 rounded-lg cursor-pointer transition-all"
                              >
                                Trigger Mock Call
                              </button>
                            </div>
                          ...
                          {mockCallState === 'calling' && (
                            <div className="text-center py-8 space-y-2">
                              ...
                              <strong className="block text-xs text-amber-400 font-mono uppercase tracking-widest mt-2">Connecting Peer...</strong>
                            </div>
                          )}

                          {mockCallState === 'active' && (
                            <div className="bg-slate-900 border border-indigo-900/50 p-4 rounded-xl space-y-3">
                              <div className="flex justify-between items-center text-[10px]">
                                ...
                                <span className="font-mono text-slate-400">Duration: {callDuration}s (Simulated Min: {Math.floor(callDuration/2)}m)</span>
                              </div>
                              
                              <div className="grid grid-cols-2 gap-2 text-center text-xs">
                                <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-850">
                                  <span className="block text-[9px] text-slate-500 uppercase font-mono">Free Tier</span>
                                  <strong className="block font-mono text-emerald-400">{mockFreeMinutes} min left</strong>
                                </div>
                                <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-850">
                                  <span className="block text-[9px] text-slate-500 uppercase font-mono">Wallet balance</span>
                                  <strong className="block font-mono text-white">₹{mockWalletBalance}</strong>
                                </div>
                              </div>

                              <button 
                                onClick={stopMockCall}
                                className="w-full bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs py-2 rounded-lg cursor-pointer"
                              >
                                End Session
                              </button>
                            </div>
                          ...
                          {mockCallState === 'ended' && (
                            <div className="bg-slate-900 p-4 rounded-xl text-center space-y-2 text-xs">
                              <CheckCircle2 className="w-8 h-8 text-rose-500 mx-auto" />
                              <strong className="block text-white">Call Disconnected</strong>
                              ...
                              <button 
                                ...
                                className="mt-2 text-indigo-400 hover:underline flex items-center gap-1 justify-center mx-auto"
                              >
                        ...
                        <div className="text-[10px] text-slate-500 leading-relaxed pt-2 border-t border-slate-900 font-medium">
                      ...
                      <div className="lg:col-span-4 bg-slate-955/50 border border-slate-850 p-6 rounded-2xl space-y-4 flex flex-col justify-between">
                        <div className="space-y-4">
                          ...
                          <div className="bg-slate-900 border border-slate-850 p-4 rounded-xl space-y-2.5 text-xs text-left">
                            <div className="flex justify-between items-center">
                              <strong className="text-white font-semibold">{kycLawyerName}</strong>
                              <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                kycStatus === 'pending' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
                                kycStatus === 'verified' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                            ...
                            <div className="space-y-1.5 text-[10px] text-slate-400 leading-relaxed font-mono bg-slate-950 p-2.5 rounded-lg border border-slate-900">
                              ...
                              {kycStatus === 'pending' ? (
                                <>
                                  <button 
                                    ...
                                    className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] py-2 rounded-lg cursor-pointer"
                                  >
                                    Approve Advocate
                                  </button>
                                  <button 
                                    onClick={handleKycReject}
                                    className="flex-1 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-rose-500 font-bold text-[10px] py-2 rounded-lg cursor-pointer"
                                  >
                                    Reject
                                  </button>
                                </>
                                ...
                                <button 
                                  ...
                                  className="w-full bg-slate-900 hover:bg-slate-850 text-slate-400 font-bold text-[10px] py-2 rounded-lg cursor-pointer flex items-center justify-center gap-1"
                                >
                                  <RotateCcw className="w-3.5 h-3.5" /> Re-trigger KYC Audit
                                </button>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="bg-slate-950 border border-slate-900 p-3 rounded-lg text-[9px] text-slate-500 h-24 overflow-y-auto space-y-1.5 font-mono">
          ...
          <div className="flex justify-between items-center border-t border-slate-850/80 pt-6 mt-6">
            ...
            <div className="flex gap-2">
              <button
                onClick={handlePrev}
                className="p-2.5 rounded-xl border border-slate-800 bg-slate-950 hover:bg-slate-900 text-slate-400 hover:text-white transition-colors cursor-pointer"
      ...
      {/* FOOTER */}
      <footer className="px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-slate-900 bg-slate-950 text-xs text-slate-400 z-20">
        <div>
          <span>LegalTalk India On-Demand Advisory Marketplace. </span>
          <span className="text-slate-600 font-mono">Confidential Pitch deck and Live sandbox system.</span>
        </div>
        <div className="flex gap-4">
          <Link to="/" className="hover:text-white transition-colors">Client Site</Link>
          <Link to="/advocates" className="hover:text-white transition-colors">Advocate Site</Link>
        </div>
      </footer>
